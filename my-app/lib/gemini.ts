...
export async function generateFullApplicantProfile(): Promise<{
  name: string;
  age: number;
  background: string;
  personality: string;
...
重要なルール:
1. "hiddenPersonality" はプレイヤーには絶対に見えない内面の性格です。この人がどういう対応をされると伸びるか/潰れるかを含めてください。例:「褒められると素直に伸びるタイプ。厳しく叱られると萎縮して逆効果。」「プライドが高いので認められたい。甘やかすとサボる。」など
2. "resume" は履歴書です。この人を雇った後の活躍ぶり（良くも悪くも）が少しだけ推測できるような内容にしてください。直接的に「この人はサボります」とは書かず、経歴や趣味、自己PRの文面から読み取れるようにしてください。150文字程度。
3. "personality" は表面的な性格で、面接前に見える情報です。20文字以内。
4. skill, speed, motivationは隠しパーソナリティと矛盾しないようにしてください。

JSON形式で返してください。余計なテキストは不要:
{"name": "姓 名", "age": 18-55の数字, "background": "経歴（50文字以内）", "personality": "表面的な性格（20文字以内）", "hiddenPersonality": "隠しパーソナリティ（80文字以内）", "resume": "履歴書内容（150文字程度）", "role": "barista or manager", "skill": 20-80の数字, "speed": 30-90の数字, "motivation": 50-90の数字, "hourlyWage": 1000-2000の数字}`;
